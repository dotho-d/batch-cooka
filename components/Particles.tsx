...
const Particles: React.FC<ParticleProps> = ({
  count = 10, // Réduit à 10 au lieu de 40
  color = '#FFF5E9',
  minSize = 1,
  maxSize = 3,
  minDuration = 15000,
  ...
  const particles = React.useMemo(() => {
    return Array.from({ length: count }).map((_, i) => {
      ...
      const delay = Math.random() * 8000; // Délai plus long pour désynchroniser
...
const Particle: React.FC<SingleParticleProps> = ({
  ...
  useEffect(() => {
    ...
    // Animation de taille plus aléatoire
    ...
    // Animation d'opacité plus aléatoire
